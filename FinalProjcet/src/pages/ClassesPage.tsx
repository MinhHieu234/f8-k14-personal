...
import {
    Box, Typography, Button, TextField, AppBar, Toolbar, Avatar,
    Link as MuiLink, Menu, MenuItem
} from '@mui/material';
...
const ClassesPage: React.FC = () => {
    ...
    const handleLogout = () => {
        handleClose();
        if (user?.email) rememberEmail.set(user.email);
        dispatch(logout());
                ...
                <Toolbar>
                    ...
                    <Button onClick={handleClick} sx={{ textTransform: 'none', color: 'text.primary', p: 0.5, borderRadius: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Avatar sx={{ width: 32, height: 32 }} src="https://i.pravatar.cc/100?img=12" />
                            <Box sx={{ textAlign: 'left' }}>
                                <Typography variant="body2" fontWeight="bold">{user?.email || 'User'}</Typography>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', mb: 3, gap: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 2 }}>
                        <TextField
                            ...
                            sx={{ width: 240, backgroundColor: 'white', borderRadius: '8px' }}
